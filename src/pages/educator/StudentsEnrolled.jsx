    ...
    <div className="px-10 py-3">
      <div className="max-w-4xl mt-5 rounded-md border border-gray-500/20">
        <table className="w-full table-auto text-gray-700">
          <thead className="border-b border-gray-300/60 rounded-xl overflow-hidden max-sm:text-xs">
            <tr>
              ...
              <th className="px-5 py-3 text-start max-sm:hidden">Date</th>
            </tr>
          </thead>
          <tbody>
            {studentEnroll.map((student, idx) => (
              <tr key={idx} className="max-sm:text-xs">
                <td className="py-3 px-5">{idx + 1}</td>
                <td className="flex items-center gap-3 py-3 px-3 mt-2">
                  <img
                    src={student.student.imageUrl}
                    alt="image"
                    className="max-sm:w-5 w-10 shadow-lg rounded-full"
                  />
